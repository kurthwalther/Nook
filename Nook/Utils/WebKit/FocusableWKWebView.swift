...
final class FocusableWKWebView: WKWebView, SessionWebView {
    ...
    override init(frame: CGRect, configuration: WKWebViewConfiguration) {
        ...
        KurthWorkflows.instalar(en: self) // kurth: grabador de workflows (quieto hasta que se graba)
    }
    ...
    override func layout() {
        ...
        guard let window = window as? MiniBrowserWindow, bounds.height > 0 else {
            // kurth: en una ventana del navegador la barra superior flota sobre la página; aquí la
            // página aprende cuánto le tapa (KurthChrome decide el inset, incluido el cero).
