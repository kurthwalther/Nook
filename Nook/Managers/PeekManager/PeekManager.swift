...
final class PeekManager {
    ...
    private(set) var page: PageSession? {
        // kurth: KurthPeekBackdrop encoge la página de atrás mientras hay Peek (y antes lo oía
        // PeekOverlayView); las notificaciones se conservan sobre el Peek nuevo de upstream.
        didSet {
            guard (page == nil) != (oldValue == nil) else { return }
            NotificationCenter.default.post(name: page == nil ? .peekDidDeactivate : .peekDidActivate, object: self)
        }
    }
